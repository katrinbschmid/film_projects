/**
 * Maya height constraint node to y constraint transforms on a mesh
 * Can handle multiple input shapes
 * https://github.com/katrinbschmid/film_projects/wiki
 * s
 ...
 *
 * example maya scene included in test folder
*/
...
const static std::string VERSION_STRING = getVersionString();
   ...
   void FrameStart(MTime & a_Time, void * a_ClientData)
   {
            ...
            it != timers.end(); ++it)
      {
         ...
         LOG(PRNT, ( "T: %20.5fms (%4d) : %s", timer.GetTotalElapsed()*1000.0f,
               timer.GetCount(), name.c_str()));
      }
...
MStatus terrainConstraintNode::initialize()
{
   ...
   CHECK_MSTATUS(attributeAffects(aPositionX, aPositionY));
   CHECK_MSTATUS(attributeAffects(aPositionZ, aPositionY));
   CHECK_MSTATUS(attributeAffects(aParent, aPositionY));
   CHECK_MSTATUS(attributeAffects(aRotPiv, aPositionY));
   CHECK_MSTATUS(attributeAffects(aRotPivTrans, aPositionY));
   CHECK_MSTATUS(attributeAffects(aOffset, aPositionY));
...
MStatus terrainConstraintNode::compute(const MPlug &plug, MDataBlock &data)
{
   ...
   if (plug == aPositionY || m_swFirstRun == 1)
   {
       ...
       if (m_swFirstRun == 1)
       {
           bool bb = m_swLookUp.setBoundingBox(meshPlug);
           m_swFirstRun = 0;
   ...
   #endif
         //ray trace look up value
         status = m_swLookUp.rayTraceLookUp(meshPlug, world[0], Y, world[2], 0);
   #ifdef ENABLE_TIMINGS
          TIMER_STOP(RTLookup)
