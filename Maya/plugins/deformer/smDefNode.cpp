...
void getAverageVector(std::vector<MVector> directionVectors,
  int neighboursSize, float &avX, float &avY, float &avZ)
{
...
MStatus smoothDeformer::deform(MDataBlock& block, MItGeometry& iter,
...
#ifdef debug
                   std::cout << g << " w: " << vWeights[g] <<" "<<iterMesh.onBoundary(&status)
                     <<" " << keepBorder << std::endl;
#endif
                }
                else
...
#ifdef debug
                    std::cout << defMulti << " : "<< minLength << scaleV <<
                      " defMulti " << avLength <<" : "<< maxLength<<std::endl;
#endif
